...
The single most reliable articulation check: **state the idea as one plain, atomic sentence, out loud or in writing, before committing to the artifact form.**
...
- For a conceptual data model or schema: verbalize the underlying facts as atomic natural-language sentences ("Each Auction is conducted at exactly one Site"). If a fact sentence is compound, or cannot be verbalized cleanly without relying on storage mechanics (foreign keys, tables, columns), the model is encoding something nobody can state — a defect that will surface later as confusion.

Failure to pass the verbalization test is diagnostic, not cosmetic. It means the problem is upstream, in the thinking or the boundary selection. Fix the idea, then return to the wording.
...
3. **Ambiguous reference.** A word or model element that could mean two things in context: "it," "this," "the service," "the old approach," or an unanchored entity missing a clear reference scheme. Each ambiguity forces the reader to guess, and some will guess wrong silently. Fix: repeat the noun, or state the explicit identifier scheme. Repetition is cheaper than ambiguity. (In code: a variable named `data` or `result`, a parameter named `flag`, a function named `process` — names that could bind to anything.)
...
## Applying to code & models specifically

Code and domain models are read far more often than written; every reading is an act of the artifact articulating itself to a person. The barriers above translate directly, plus:
...
- **Interfaces should be simpler than implementations.** A caller should learn less to use a module than the author learned to write it. If using a function or a model correctly requires knowing its internal storage engine or mechanics, the module is leaking, not articulating.
- **Cohesion is felt as intuition.** When a module or model is cohesive, its parts interact in ways a reader can predict before looking — the second function is where you expect, doing what its name says. That intuitive feel is the audience-facing evidence of a well-articulated boundary. When a reader keeps being surprised, the boundary is wrong: move the surprising element to where the reader expected it, or rename the module to state what it actually contains.
- **Commit messages, PR descriptions, and schema docs are prose artifacts.** Apply the full taxonomy: lede first (what changed and why), no ambiguous references ("fixed the bug" — which?), no missing bridges (why this approach over the obvious one).

## Creation mode

While producing any artifact:

1. Establish the audience contract before drafting. If the request does not specify audience or intended action, decide by impact: ask when the answer would materially change the artifact; otherwise state your assumption explicitly at the top of the work so it can be corrected.
2. Pass the verbalization test before structuring. Write the one-sentence version first — atomic and plain — and let it become the lede.
...
- **Type** (from the taxonomy) — location — why it stops this
  audience — a concrete suggested fix, with rewritten text,
  verbalized facts, or renamed identifiers where practical.]

## Verdict
[One of: (a) Wording problems — the thinking and abstractions are
sound, apply the fixes; (b) Boundary/Abstraction problems — the
thinking is clear but presented at the wrong layer or scope for
this audience; (c) Thinking problems — core concepts or decisions
are missing or unformed, and which ones; (d) Sound — the artifact
articulates clearly, with at most minor polish.]
```
...
Requirements gathering is articulation in reverse: the idea lives in the stakeholder's head, often unformed, and the job is to help them extract it with minimal loss. The barriers stop being findings and become probes — each barrier type has a corresponding question that converts fuzz into fact.
...
Formatting and conventions skills (structure templates, BLUF, diagram standards, frontmatter rules) are peers, not competitors. Conventions make artifacts consistent; articulation makes them land. Apply both when both are loaded: follow the conventions for form, and use this skill to judge whether the content inside that form actually transfers the idea. When they conflict — a template section that would force a wall of detail, say — the audience contract wins: adapt the form and note the deviation.
