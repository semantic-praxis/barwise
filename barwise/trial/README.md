...
package's tests, in the fixing PR. The gate checks all three.

A step the lane could not answer (`could_not_answer`) is an open item
too, not a pass. It needs a baseline row with a `note` saying why it
cannot be answered and an `issue` tracking the gap, and it carries
`"status": "could_not_answer"`; it needs no reproduction, since there is
no defect to reproduce. A new unanswerable step makes the gate exit 2,
"could not answer". A new failure, a stale row, or a row missing its
note, issue or reproduction makes it exit 1, and exit 1 wins when both
happen, because a found regression is the more actionable answer.
