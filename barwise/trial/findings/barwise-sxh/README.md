# barwise-sxh: the OpenAPI export writes a comment header into JSON

```sh
barwise export trial/findings/barwise-sxh/one-reading.orm.yaml --format openapi --output /tmp/one.json
head -3 /tmp/one.json
