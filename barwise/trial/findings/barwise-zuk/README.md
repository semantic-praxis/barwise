# barwise-zuk: the `ddl` importer is one regex

```sh
barwise import model trial/findings/barwise-zuk/messy.sql --format ddl
```
...
makes the DDL round trip re-import with zero fact types (barwise-dnm):
