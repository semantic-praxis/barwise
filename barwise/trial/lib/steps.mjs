...
export function sprint1Brownfield(customer, tier, record) {
  ...
  for (const art of customer.artifacts ?? []) {
    ...
    const vb = runCli(["validate", back, "--format", "json"], { timeoutMs: budget });
    record({
      sprint: 1,
      step: `roundtrip-validate:${art.id}`,
      artifact: art.id,
      importer: art.importer,
      format: exporter,
      ...grade.gradeProducedModelValidation(vb, parseJson(vb.stdout)),
      ms: vb.ms,
      exit: vb.exit,
    });
    const d = runCli(["diff", out, back, "--format", "json"], { timeoutMs: budget });
    const diff = parseJson(d.stdout);
    const outcome2 = d.exit === 0
...
/**
 * The documented non-zero outcomes. Each replaces an `allowNonZero: true`
 * that accepted ANY non-zero exit, so a genuine command error exiting 1
 * without a stack trace was recorded as a pass. A caller now names the one
 * result it expects; anything else is still a finding.
 */
const EXPECTED_NONZERO = {
  queryPath: /no path|not connected|no route|unreachable/i,
  lineageNoManifest: /no manifest|not found|run .*export|never exported/i,
  lineageUntracked: /not tracked|no artifacts|unknown element|not found/i,
  mergeStructuralError: /structural error|conflict/i,
};
...
export function sprint1ModelOps(customer, tier, record) {
  ...
  for (
    ...
    record({
      sprint: 1,
      step: `model-roundtrip-validate:${fmt}`,
      format: fmt,
      ...grade.gradeProducedModelValidation(vb, parseJson(vb.stdout)),
      ms: vb.ms,
      exit: vb.exit,
    ...
    record({
      ...
      step: `model-roundtrip:${fmt}`,
      ...(d.exit === 0
        ? grade.gradeRoundTrip(diff, lossSet(fmt))
        : grade.gradeCommand(d, { budgetMs: budget })),
      ms: d.ms,
      exit: d.exit,
    });
...
export function sprint3Governance(customer, tier, record) {
  ...
  // Every object type the split declares in a context mapping. A type in
  // two domains is the documented seam behaviour when a mapping names it,
  // and a silent replication when none does.
  const mapped = new Set();
  const mappingsDir = join(outDir, "mappings");
  for (const f of existsSync(mappingsDir) ? readdirSync(mappingsDir) : []) {
    const m = parse(readFileSync(join(mappingsDir, f), "utf8"))?.mapping ?? {};
    for (const e of m.entity_mappings ?? []) {
      if (e.source_object_type) mapped.add(e.source_object_type);
      if (e.target_object_type) mapped.add(e.target_object_type);
    }
  }
  record({
    sprint: 3,
    step: "split",
    ...grade.gradeSplit(all, domainNames, res.stderr + res.stdout, mapped),
    ms: res.ms,
    exit: res.exit,
    stderr: tail(res.stderr),
  });
  ...
  if (first.length >= 2) {
    ...
    record({
      sprint: 3,
      step: "query-path",
      ...grade.gradeCommand(q, { budgetMs: budget, expectedNonZero: EXPECTED_NONZERO.queryPath }),
      ms: q.ms,
      exit: q.exit,
...
export function sprint4Downstream(customer, tier, record) {
  ...
  for (const dialect of dialects) {
    ...
    if (rg.status !== "pass") continue;
    const vd = runCli(["validate", back, "--format", "json"], { timeoutMs: budget });
    record({
      sprint: 4,
      step: `read-back-validate-ddl:${dialect}`,
      dialect,
      importer: "sql",
...
export function sprint4bLateRequirement(customer, tier, record) {
  ...
  record({
    ...
    ...grade.gradeCommand(fresh, {
      budgetMs: budget,
      expectedNonZero: EXPECTED_NONZERO.lineageNoManifest,
    }),
    ms: fresh.ms,
    exit: fresh.exit,
  ...
  record({
    ...
    change: late.change,
    ...(d.exit === 0
      ? grade.gradeHistoryStep(applied.expect, diff)
      : grade.gradeCommand(d, { budgetMs: budget })),
    ms: d.ms,
    exit: d.exit,
  });
...
export function sprint5ChangeStorm(customer, tier, record) {
  ...
  for (let i = 1; i < built.steps.length; i++) {
    ...
    const mg = grade.gradeCommand(m, {
      budgetMs: budget,
      expectedNonZero: step.change.kind === "remove_object_type"
        ? EXPECTED_NONZERO.mergeStructuralError
        : null,
    });
    record({
      sprint: 5,
  ...
  try {
    await withMcp(async (mcp) => {
      ...
      for (const [name, args, tool, toolArgs, compare] of pairs) {
        ...
        if (m.isError || cli.exit !== 0) {
          record({
            sprint: 6,
            step: `parity:${name}`,
            status: "could_not_answer",
            detail: m.isError
              ? `the MCP tool returned an error, so there is no answer to compare: ${
                String(m.text).split("\n")[0].slice(0, 160)
              }`
              : `the CLI exited ${cli.exit}, so there is no answer to compare`,
            ms: m.ms,
            exit: cli.exit,
          });
          continue;
