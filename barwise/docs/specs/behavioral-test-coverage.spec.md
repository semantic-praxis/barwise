...
## Finding: the example outputs were untracked local scratch (resolved)

`examples/output/` holds `*.verbalizations.txt` and `*.diagnostics.txt`
for models extracted from the sample transcripts, but the directory was
gitignored, so the files were never committed -- a local copy left by an
older CLI build had drifted (the diagnostics even used an old
`[warning] <ruleId>: <msg>` format the current CLI no longer prints),
giving a false impression of staleness in the repo when in fact nothing
was tracked. This characterization net is built fresh and does not rely
on `examples/output/`.

Resolved separately: `examples/output/` is now tracked as committed
comparison artifacts, regenerated by `npm run regen:examples` and stamped
with the barwise version, so cross-version diffs of verbalization and
validation are explicit. The committed safety net for the refactor is the
golden characterization under `packages/cli/tests/`, not these samples.
