...
Tracking: barwise-1021 through barwise-1034 are the findings the first
...
| ------------ | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| barwise-1021 | `import sql` returns the targets of FOREIGN KEY clauses, not the tables; 24% to 100% of tables dropped without a warning | S1                                           |
| barwise-1022 | `import model --format ddl` is one regex: schema-qualified, bracketed, backticked and unterminated statements all miss   | S3                                           |
| barwise-1023 | The OpenAPI and DDL importers write duplicate role ids, so every model they produce fails validation                     | S1                                           |
| barwise-1024 | The OpenAPI export writes a `/* */` header into a .json file, which nothing can parse back                               | S1                                           |
| barwise-1025 | DDL export then import is not a fixed point; the annotated export re-imports with zero fact types                        | S1                                           |
| barwise-1026 | The dbt importer throws when two relationship tests derive one reading, which its own export produces                    | S3                                           |
| barwise-1027 | dbt round trip loses value types and value constraints beyond the declared loss set                                      | S1                                           |
| barwise-1028 | NORMA round trip re-homes subset, exclusion and external-uniqueness constraints onto sibling fact types                  | S1                                           |
| barwise-1029 | `diff` reports six of fourteen object-type renames as a removal plus an addition, with no stated rule                    | S5                                           |
| barwise-1030 | The Kotlin importer returns an empty model for data classes and sealed classes                                           | S1                                           |
| barwise-1031 | `merge` exits 1 with "1 structural error(s)" on a fact-type rename and does not say which                                | S3                                           |
| barwise-1032 | The release CLI bundle cannot find its own gym catalog: `gym list` fails with a Node path error                          | S3                                           |
| barwise-1033 | `barwise diff a b                                                                                                        | head` ends in an unhandled EPIPE stack trace | S2 |
| barwise-1034 | OpenAPI round trip drops every value type's length, definition and value constraint                                      | S1                                           |

Four of these are what the trial was built to find and nothing else
would have: barwise-1021 and barwise-1030 are importers that exit zero
on an empty or partial result, barwise-1024 and barwise-1032 are
...
| 2    | Fixes for the S1 importers (barwise-1021, 1023, 1030)       | `trial:offline` per PR; baseline rows come out      | Next                                     |
