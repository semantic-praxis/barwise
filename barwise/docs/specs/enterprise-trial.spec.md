...
Tracking: barwise-jjd through barwise-a0h are the findings the first
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------- |
| barwise-jjd | `import sql` returns the targets of FOREIGN KEY clauses, not the tables; 24% to 100% of tables dropped without a warning | S1                                           |
| barwise-zuk | `import model --format ddl` is one regex: schema-qualified, bracketed, backticked and unterminated statements all miss   | S3                                           |
| barwise-lh9 | The OpenAPI and DDL importers write duplicate role ids, so every model they produce fails validation                     | S1                                           |
| barwise-sxh | The OpenAPI export writes a `/* */` header into a .json file, which nothing can parse back                               | S1                                           |
| barwise-dnm | DDL export then import is not a fixed point; the annotated export re-imports with zero fact types                        | S1                                           |
| barwise-bvl | The dbt importer throws when two relationship tests derive one reading, which its own export produces                    | S3                                           |
| barwise-vsp | dbt round trip loses value types and value constraints beyond the declared loss set                                      | S1                                           |
| barwise-ro8 | NORMA round trip re-homes subset, exclusion and external-uniqueness constraints onto sibling fact types                  | S1                                           |
| barwise-rkm | `diff` reports six of fourteen object-type renames as a removal plus an addition, with no stated rule                    | S5                                           |
| barwise-dyq | The Kotlin importer returns an empty model for data classes and sealed classes                                           | S1                                           |
| barwise-nqd | `merge` exits 1 with "1 structural error(s)" on a fact-type rename and does not say which                                | S3                                           |
| barwise-fbi | The release CLI bundle cannot find its own gym catalog: `gym list` fails with a Node path error                          | S3                                           |
| barwise-y6d | `barwise diff a b                                                                                                        | head` ends in an unhandled EPIPE stack trace | S2 |
| barwise-a0h | OpenAPI round trip drops every value type's length, definition and value constraint                                      | S1                                           |

Four of these are what the trial was built to find and nothing else
would have: barwise-jjd and barwise-dyq are importers that exit zero
on an empty or partial result, barwise-sxh and barwise-fbi are
...
- **The lane stopped carrying its own format validators.** Sprint 4
  ...
  inventing one. The rewrite immediately found barwise-oph, which the
...
| 2    | Fixes for the S1 importers (barwise-jjd, 1023, 1030)        | `trial:offline` per PR; baseline rows come out      | Next                                     |
