/**
 * MCP project loader: the filesystem walk for a multi-file ORM project.
 *
 ...
 * references (resolved relative to the manifest directory), then hands the
 * contents to the pure assembler. This mirrors the CLI's loader -- the two
 * packages do not depend on each other, so a small amount of parallel I/O
 * code is preferable to coupling them (DRY is secondary here).
 */
