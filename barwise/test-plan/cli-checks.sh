...
# A command, not a path -- so it lives in an array. Splitting the
# override once, here, is what lets every call site quote normally;
# holding it as a string forced 21 unquoted expansions and a
# file-wide SC2086 suppression that blinded the file to real ones.
read -ra BARWISE_CMD <<< "${BARWISE:-node ${BUNDLE}}"

# No override given means we are about to run the default bundle, so
# it had better exist. (This used to compare BARWISE against the
# default string, which only worked because the assignment above had
# already defaulted it -- `read -ra` does not, so the test says what
# it means now.)
if [[ -z "${BARWISE:-}" && ! -f "${BUNDLE}" ]]; then
...
run() {
  shift; [[ "$1" = "--" ]] && shift  # the description is the caller's; assertions carry their own
  OUT="$("$@" 2>&1)"; RC=$?
}
...
section "Smoke"
run "version" -- "${BARWISE_CMD[@]}" --version;        assert_exit "barwise --version" 0
run "help"    -- "${BARWISE_CMD[@]}" --help;            assert_ok_contains "barwise --help lists commands" "validate"
...
section "Validate"
run "v-clean" -- "${BARWISE_CMD[@]}" validate "${CLEAN}";          assert_ok_contains "validate clean model -> 0 errors" "0 error"
run "v-json"  -- "${BARWISE_CMD[@]}" validate "${CLEAN}" --format json
...
run "v-broken" -- "${BARWISE_CMD[@]}" validate "${BROKEN}";        assert_exit "validate broken model exits 1" 1

section "Verbalize"
run "verb" -- "${BARWISE_CMD[@]}" verbalize "${CLEAN}";   assert_exit "verbalize clean model" 0
run "ce"   -- "${BARWISE_CMD[@]}" verbalize "${SHOWCASE}" --counterexamples
...
for q in entities fact-types constraints stats anchors; do
  run "q-${q}" -- "${BARWISE_CMD[@]}" query "${CLEAN}" "${q}"; assert_exit "query ${q}" 0
done
run "q-anchors-content" -- "${BARWISE_CMD[@]}" query "${CLEAN}" anchors
...
for f in ddl openapi avro dbt; do
  run "x-${f}" -- "${BARWISE_CMD[@]}" export "${CLEAN}" --format "${f}"; assert_exit "export --format ${f}" 0
done

section "Diagram"
run "diag" -- "${BARWISE_CMD[@]}" diagram "${CLEAN}"; assert_ok_contains "diagram emits SVG" "<svg"

section "Describe"
run "desc" -- "${BARWISE_CMD[@]}" describe "${CLEAN}"; assert_exit "describe" 0

section "Diff"
run "diff" -- "${BARWISE_CMD[@]}" diff \
...
run "eu-ok"  -- "${BARWISE_CMD[@]}" validate "${EU_OK}";  assert_ok_contains "clean combination -> 0 errors" "0 error"
run "eu-bad" -- "${BARWISE_CMD[@]}" validate "${EU_BAD}"
assert_contains "shared combination -> violation" "External uniqueness constraint is violated"
run "eu-bad-rc" -- "${BARWISE_CMD[@]}" validate "${EU_BAD}"; assert_exit "violation exits 1" 1
run "eu-ce" -- "${BARWISE_CMD[@]}" verbalize "${EU_OK}" --counterexamples
...
if [[ -f "${ROOT}/docs/auction.orm.yaml" ]] && [[ -f "${EX}/auction-split.yaml" ]]; then
  run "split" -- "${BARWISE_CMD[@]}" project split "${ROOT}/docs/auction.orm.yaml" \
    --config "${EX}/auction-split.yaml" --out "${TMP}/proj"
  assert_exit "project split" 0
  run "split-validate" -- "${BARWISE_CMD[@]}" validate "${TMP}/proj/project.orm-project.yaml"
...
if [[ -n "${ANTHROPIC_API_KEY:-}" ]] && [[ -f "${TRANSCRIPT}" ]]; then
  ...
  run "import" -- "${BARWISE_CMD[@]}" import transcript "${TRANSCRIPT}" \
  ...
  if [[ -f "${OUTYAML}" ]]; then
    run "import-validate" -- "${BARWISE_CMD[@]}" validate "${OUTYAML}"
