...
function main() {
  ...
  if (mode === "--write") {
    // Refuses when a spec is untracked: invisible to trackedFiles(), so the
    // baseline would freeze a corpus missing it. --check deliberately does NOT
    // refuse -- its input is the tracked corpus by definition. The rule and the
    // incident behind it live in lib/tracked.mjs, shared with audit-spec-status.
    refuseUntrackedInput({
      pathspec: "barwise/docs/specs",
      suffix: ".spec.md",
      gate: "audit-corrections",
    });
